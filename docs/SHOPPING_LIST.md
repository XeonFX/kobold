...
*Rev 2 — reprioritised after hardware verification. The PD source module is gone (your GPIO power
path works), and the lidar is demoted from essential to optional now that a stereo rig costs €0.*

Prices are rough EU/botland estimates. Tiers are ordered so you can stop at any point and still have
...
Small, cheap, and each one prevents a specific failure you're currently exposed to.
...
| **Main kill switch, 15 A+** | 1 | €4 | On the motor rail. The thing you grab when it does something unexpected |
| **XT30/XT60 connectors + 18 AWG silicone wire** | 1 set | €10 | Dupont jumpers on a motor rail is how you get melted connectors |
...
| **2D lidar — LD19/LD06 or RPLIDAR C1** | 1 | €70–90 | **Promoted back to essential.** The €0 stereo plan is dead — the ROCK 5B has one CSI connector, and your two IMX219s have mismatched lenses. Mono depth on the NPU covers 3D obstacles, but localisation needs this. Lidar works in the dark, against blank walls, and over plain carpet, and needs no calibration |
...
| Item | ~Price | Why |
|---|---|---|
| **Wide-angle M12 lens for IMX219** | €10 | Only if both your modules turn out narrow after the FOV test (PROJECT_PLAN §6). Wider FOV directly improves visual place recognition |
| **2× matched USB webcams** | €30 | The sane route to stereo if you still want it — no CSI limits, no device tree overlay work. Rolling shutter and no hardware sync, but it works |
| **OAK-D Lite** | €150 | The expensive route to stereo. Does depth on-device, so it costs you no RK3588 CPU at all. Genuinely good, but the lidar plus mono depth gets you most of the way for half the price |

---

## Tier 3 — voice hardware, when you're ready (~€80–100)

You explicitly deferred this and the phone app covers it. When you buy, nothing above the `voice`
...
| Item | ~Price | Why |
|---|---|---|
| **Charging dock** (contacts + IP2721 + buck + Zero 3W) | €20 | Uses parts you own. "Go charge yourself" is the difference between a project and a robot that lives in your house |
...
| ~~3S balance charger~~ | **Resolved** — your BMSes balance |
...
| A CSI splitter + second IMX219 for MIPI stereo | One CSI connector *can* split to 2×2-lane, but there's no official adapter, no documented overlay, and you'd be writing a dual-sensor device tree yourself. Research project, not a build step |
...
*See [COMPONENTS.md](COMPONENTS.md) for what you already own and
